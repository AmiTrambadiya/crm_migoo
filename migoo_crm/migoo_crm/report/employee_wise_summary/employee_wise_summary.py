...
def execute(filters=None):
    ...
    data = []
    start_date = datetime.strptime(filters.get('start_date'), '%d-%m-%Y').date() if filters.get('start_date') else datetime.now().date().replace(day=1, month=1) - timedelta(days=7)
    end_date = datetime.strptime(filters.get('end_date'), '%d-%m-%Y').date() if filters.get('end_date') else datetime.now().date()
    ...
    if filters.get('period') == 'Daily':
        while start_date <= end_date:
            ...
            data = sorted(data, key=lambda x: datetime.strptime(x['period'], '%d-%m-%Y'), reverse=True)
        ...
        while start_date <= end_date:
            ...
            data.sort(key=lambda x: datetime.strptime(x["period"].split(" - ")[0], "%d-%m-%Y"), reverse=True)
    ...
    #    start_date = datetime(current_date.year - 1, 1, 1).date()
       ...
       end_date = (current_date + relativedelta(months=1)).replace(day=1) - timedelta(days=1)
       while start_date <= end_date:
        year_month = start_date.strftime('%m-%Y')
        month_start = datetime.strptime(year_month + '-01', '%m-%Y-%d').date()
        ...
        data = sorted(data, key=lambda x: datetime.strptime(x['period'], '%m-%Y'), reverse=True)
