...
class Lead(SellingController, CRMNote):
    ...
    def on_trash(self):
        frappe.db.sql(
            """update `tabIssue` set lead='' where lead=%s""", self.name)
    ...
    def set_full_name(self):
        if self.first_name:
            self.lead_name = " ".join(
                filter(None, [self.salutation, self.first_name,
    ...
    def set_lead_name(self):
        if not self.lead_name:
            ...
            if not self.company_name and not self.email_id and not self.flags.ignore_mandatory:
                frappe.throw(
                    _("A Lead requires either a person's name or an organization's name"))
    ...
    def validate_email_id(self):
        if self.email_id:
            ...
            if self.email_id == self.lead_owner:
                frappe.throw(
                    _("Lead Owner cannot be same as the Lead Email Address"))
        ...
        if self.contact_doc:
            self.contact_doc.append(
                "links", {"link_doctype": "Lead",
                          "link_name": self.name, "link_title": self.lead_name}
            )
    ...
    def has_quotation(self):
        return frappe.db.get_value(
            "Quotation", {"party_name": self.name,
    ...
    def create_contact(self):
        ...
        if self.email_id:
            contact.append(
                "email_ids", {"email_id": self.email_id, "is_primary": 1})

        if self.phone:
            contact.append(
                "phone_nos", {"phone": self.phone, "is_primary_phone": 1})
    ...
    def create_prospect(self, company_name):
        try:
            ...
            frappe.throw(_("Prospect {0} already exists").format(
...
def make_lead_from_communication(communication, ignore_communication_links=False):
    ...
    link_communication_to_document(
        doc, "Lead", lead_name, ignore_communication_links)
    return lead_name
