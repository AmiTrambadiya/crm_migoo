...
class AutoEmailReport(Document):
    ...
    def validate_report_count(self):
        count = frappe.db.count("Auto Email Report", {
                                "user": self.user, "enabled": 1})
        ...
        if count > max_reports_per_user + (-1 if self.flags.in_insert else 0):
            msg = _("Only {0} emailed reports are allowed per user.").format(
                max_reports_per_user)
            msg += " " + \
                _("To allow more reports update limit in System Settings.")
        ...
        if self.format not in valid_report_formats:
            frappe.throw(
                _("{0} is not a valid report format. Report format should one of the following {1}").format(
                    frappe.bold(self.format), frappe.bold(
                        ", ".join(valid_report_formats))
                )
            )
    ...
    def validate_mandatory_fields(self):
        ...
        filter_meta = frappe.parse_json(
            self.filter_meta) if self.filter_meta else {}
        throw_list = []
    ...
    def get_report_content(self):
        ...
        # add serial numbers
        columns.insert(0, frappe._dict(
            fieldname="idx", label="", width="30px"))
    ...
    def get_html_table(self, columns=None, data=None):
        ...
        report_doctype = frappe.db.get_value(
            "Report", self.report, "ref_doctype")
    ...
    def prepare_dynamic_filters(self):
        ...
        from_date = add_to_date(
            to_date, **{from_date_value[0]: from_date_value[1]})
    ...
    for row in data:
        ...
        for col in columns:
            ...
            if col.fieldtype == "Link":
                ...
                doc = frappe.get_doc(col.parent, doc_name) if doc_name and col.get(
                    "parent") else None
